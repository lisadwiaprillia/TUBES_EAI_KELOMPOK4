...
$(document).on("hidden.bs.modal", "#addTask", function () {
  $(this).find("form").trigger("reset");
});

$(document).on("hidden.bs.modal", "#editTask", function () {
  $(this).find("form").trigger("reset");
});
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="text-xs font-weight-bold">${
                                      val.responsible_user ? val.responsible_user : "--"
                                    }</span>
                                </td>
                                <td>
                                    <span class="badge badge-sm bg-gradient-${
                                      status_color ? status_color : status_color
                                    }">${status_value ? status_value : "--"}</span>                                       
                                </td>                                   
                                ...
                                <td>
                                    <div data-toggle="modal" data-target="#editTask" style="display: inline">
                                        <a class="btn btn-link text-dark px-3 mb-0" href="javascript: editTask(${
                                          val.id
                                        });"  ><i class="fas fa-pencil-alt text-dark me-2" aria-hidden="true"></i></a>
                                    </div>                                                                              
                                    <a class="btn btn-link text-danger text-gradient px-3 mb-0" href="javascript: deleteTask(${
                                      val.id
      ...
      .catch((error) => {
        $("#showTask").html(`
                    <tr class="text-center">
                        <td colspan="10" class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Data Not Found...</td>
                    </tr>
                `);
        if (error.status === 401) {
